...
async function main() {
  ...
  const masterChefV3 = await MasterChefV3.deploy(config.kfc, positionManager_address, config.WNATIVE);

  console.log("masterChefV3 deployed to:", masterChefV3.address);
  // await tryVerify(masterChefV3, [config.kfc, positionManager_address]);
