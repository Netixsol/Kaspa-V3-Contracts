...
import '../../../../../contracts/KaspaV3Pool.sol';
...
contract SetupUniswap {
    ...
    KaspaV3Factory factory;

    constructor(TestERC20 _token0, TestERC20 _token1) public {
        factory = new KaspaV3Factory();
        token0 = _token0;
        token1 = _token1;
    }

    function createPool(uint24 _fee, uint160 _startPrice) public {
        pool = KaspaV3Pool(factory.createPool(address(token0), address(token1), _fee));
...
contract UniswapMinter {
    ...
    function setPool(KaspaV3Pool _pool) public {
        pool = _pool;
    }
