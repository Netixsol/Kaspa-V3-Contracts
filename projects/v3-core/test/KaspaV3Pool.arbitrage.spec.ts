...
import { KaspaV3PoolSwapTest } from '../typechain-types/contracts/test/KaspaV3PoolSwapTest'
...
describe('KaspaV3Pool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('KaspaV3PoolSwapTest')
            const tester = (await testerFactory.deploy()) as KaspaV3PoolSwapTest
          ...
          let tester: KaspaV3PoolSwapTest
