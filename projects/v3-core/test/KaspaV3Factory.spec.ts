...
describe('KaspaV3Factory', () => {
  let wallet: Wallet, other: Wallet

  let deployer: KaspaV3PoolDeployer
  ...
  const fixture = async () => {
    ...
    const deployer_ = (await deployerFactory.deploy()) as KaspaV3PoolDeployer
    const factory_ = (await factoryFactory.deploy(deployer_.address)) as KaspaV3Factory

    await deployer_.setFactoryAddress(factory_.address)
    return [factory_, deployer_] as [KaspaV3Factory, KaspaV3PoolDeployer]
  }
  ...
  before('load pool bytecode', async () => {
    poolBytecode = (await ethers.getContractFactory('KaspaV3Pool')).bytecode
  })
  ...
  async function createAndCheckPool(
    ...
    const poolContractFactory = await ethers.getContractFactory('KaspaV3Pool')
