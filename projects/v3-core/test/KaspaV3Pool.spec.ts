...
import { TestKaspaV3SwapPay } from '../typechain-types/contracts/test/TestKaspaV3SwapPay'
...
import { TestKaspaV3ReentrantCallee } from '../typechain-types/contracts/test/TestKaspaV3ReentrantCallee'
...
describe('KaspaV3Pool', () => {
  ...
  let factory: KaspaV3Factory
  let pool: MockTimeKaspaV3Pool

  let swapTarget: TestKaspaV3Callee
  ...
  async function initializeAtZeroTick(pool: MockTimeKaspaV3Pool): Promise<void> {
    await pool.initialize(encodePriceSqrt(1, 1))
  ...
  describe('#lock', () => {
    ...
    it('cannot reenter from swap callback', async () => {
      const reentrant = (await (
        await ethers.getContractFactory('TestKaspaV3ReentrantCallee')
      ).deploy()) as TestKaspaV3ReentrantCallee
  ...
  describe('swap underpayment tests', () => {
    let underpay: TestKaspaV3SwapPay
    beforeEach('deploy swap test', async () => {
      const underpayFactory = await ethers.getContractFactory('TestKaspaV3SwapPay')
      underpay = (await underpayFactory.deploy()) as TestKaspaV3SwapPay
