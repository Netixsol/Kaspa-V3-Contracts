...
```solidity
struct KaspaV3Pool.Slot0 slot0
```
...
```solidity
struct KaspaV3Pool.ProtocolFees protocolFees
```
...
```solidity
contract IKaspaV3LmPool lmPool
```
...
_Prevents calling a function from anyone except the address returned by IKaspaV3Factory#owner()_
...
_The caller of this method receives a callback in the form of IKaspaV3SwapCallback#kaspaV3SwapCallback_
...
_The caller of this method receives a callback in the form of IKaspaV3FlashCallback#kaspaV3FlashCallback
...
```solidity
function setLmPool(contract IKaspaV3LmPool _lmPool) external
```
