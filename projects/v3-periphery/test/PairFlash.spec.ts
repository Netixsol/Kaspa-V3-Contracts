...
describe('PairFlash test', () => {
  ...
  describe('flash', () => {
    it('test correct transfer events', async () => {
      ...
      const pool1 = computePoolAddress(deployer.address, [token0.address, token1.address], FeeAmount.MEDIUM)
      const pool2 = computePoolAddress(deployer.address, [token0.address, token1.address], FeeAmount.LOW)
      const pool3 = computePoolAddress(deployer.address, [token0.address, token1.address], FeeAmount.HIGH)
