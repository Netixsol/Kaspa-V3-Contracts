import { abi as IKaspaV3PoolABI } from '@kasplex/v3-core/artifacts/contracts/interfaces/IKaspaV3Pool.sol/IKaspaV3Pool.json'
...
describe('NonfungiblePositionManager', () => {
  ...
  const nftFixture: Fixture<{
    ...
    const { weth9, factory, tokens, nft, router, deployer } = await completeFixture(wallets, provider)
  ...
  beforeEach('load fixture', async () => {
    ;({ nft, factory, tokens, weth9, router, deployer } = await loadFixture(nftFixture))
  })
  ...
  describe('#createAndInitializePoolIfNecessary', () => {
    ...
    it('works if pool is created and initialized', async () => {
      ...
      await factory.createPool(tokens[0].address, tokens[1].address, FeeAmount.MEDIUM)
      const pool = new ethers.Contract(expectedAddress, IKaspaV3PoolABI, wallet)
  ...
  describe('multicall exit', () => {
    ...
    it('executes all the actions', async () => {
      const pool = poolAtAddress(
        computePoolAddress(deployer.address, [tokens[0].address, tokens[1].address], FeeAmount.MEDIUM),
        wallet
      )
      await expect(
        exit({
  ...
  describe('fees accounting', () => {
    ...
    describe('10k of token0 fees collect', () => {
      ...
      it('expected amounts', async () => {
        ...
        expect(nft1Amount0).to.eq(1419)
        expect(nft1Amount1).to.eq(0)
        expect(nft2Amount0).to.eq(4258)
      ...
      it('actually collected', async () => {
        ...
        await expect(
          ...
          .to.emit(tokens[0], 'Transfer')
          .withArgs(poolAddress, wallet.address, 1419)
          .to.not.emit(tokens[1], 'Transfer')
        await expect(
          nft.collect({
            tokenId: 2,
          ...
          .to.emit(tokens[0], 'Transfer')
          .withArgs(poolAddress, wallet.address, 4258)
          .to.not.emit(tokens[1], 'Transfer')
      })
    })
  })
